...
public class GPTCaller {
	...
	private static final String USER_CONTENT = 
			"Review the following error message and provide the requested information. Message: ";
	...
	public JSONArray assembleApiInstruction() {
		...
		messages.put(new JSONObject()
				.put("role", "user")
				.put("content", USER_CONTENT + normalizedInput));
	...
	public static GPTCaller init(String rawInput) {
		...
		GPTClient client = sandboxFlag.isPresent() && sandboxFlag.get() 
				? new GPTSandboxClient() 
				: new GPTHttpClient();
